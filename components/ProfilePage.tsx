...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, savedLeases, onViewLease, onSignOut, showToast }) => {
  ...
  const handleManageSubscription = () => {
      ...
      showToast("Subscription management is not yet implemented.", "success");
  }
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-slate-200">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 lowercase font-sans">My Account</h2>
          <p className="text-slate-500">Viewing account for <span className="font-semibold text-slate-600">{user.email}</span>.</p>
        </div>
        <button
            onClick={onSignOut}
      ...
      <div className="mb-8 p-4 bg-slate-50 rounded-lg border border-slate-200">
        <h3 className="text-lg font-bold text-slate-900 lowercase font-sans">Subscription Plan</h3>
        <div className="flex items-center justify-between mt-2">
            <p className="text-slate-600">You are currently on the <span className="font-semibold capitalize text-brand-primary">{user.subscriptionStatus}</span> plan.</p>
            {user.subscriptionStatus === 'free' ? (
                <button onClick={handleUpgradeClick} className="bg-brand-accent hover:bg-amber-500 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm">Upgrade to Pro</button>
            ) : (
                <button onClick={handleManageSubscription} className="bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-2 px-4 rounded-lg transition-colors text-sm">Manage Subscription</button>
            )}
        </div>
      </div>


      <h3 className="text-2xl font-bold text-slate-900 lowercase font-sans mb-4">My Saved Leases</h3>
          ...
          savedLeases.map(lease => (
            <div key={lease.id} className="flex items-center p-4 bg-slate-50 rounded-lg border border-slate-200 hover:shadow-sm transition-shadow">
              <div className="flex-shrink-0 text-brand-secondary">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-1 4h-4m2-10V4M5 8h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V10a2 2 0 012-2z" /></svg>
              </div>
